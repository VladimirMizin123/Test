        ...
        parsedCustomizations = signalRResult.map<Customization>((item) {
          ...
          List<opt.Option> options = (item['Items'] as List).map<opt.Option>((optItem) {
            try {
              ...
              final quantity = optItem['quantity'] ?? 0;
              ...
              final optionObject = opt.Option(
                ...
                quantity: quantity is num ? quantity : int.tryParse(quantity.toString()) ?? 0,
              );
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (bool didPop, Object? result) async {
        ...
        try {
          ...
          if (context.mounted) setState(() => _isGoingBack = false);
        }
      },
      child:Scaffold(
                ...
                children: [
                  GestureDetector(
                    onTap: () async {
                      final signalR = new SignalRService();
                       setState(() {
                          isQuantityLoading[option.name!] = true;
                        });
                      var payload = {
                        "Header": cs.name,
                        "Items": [option.name]
                      ...
                      setState(() {
                        ...
                        isQuantityLoading[option.name!] = false;
                      });
                    },
                    child: Container(
                      height: 30.h,
                  ...
                  Container(
                    ...
                    child: Center(
                      child: isQuantityLoading[option.name] == true
                          ? SizedBox(
                              height: 16,
                              width: 16,
                              child: CircularProgressIndicator(strokeWidth: 2),
      ...
      try {
        ...
        final parsedNestedCustomizations = signalRResult.map<Customization>((item) {
          ...
          List<opt.Option> options = (optionsList as List).map<opt.Option>((optItem) {
            print(optItem);
            if(optItem['Name'] == 'American Cheese') {
              print(optItem);
              print('-========');
            }
