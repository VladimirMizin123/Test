...
class UserSurveyBloc extends Bloc<UserSurveyEvent, UserSurveyState> {
  ...
  _onGetAllRestriction(
      GetAllRestrictionEvent event, Emitter<UserSurveyState> emit) async {
  ...
  _onGetDietPlan(GetDietPlanEvent event, Emitter<UserSurveyState> emit) async {
    emit(GetDietPlanLoadingState());
    try {
      final response = await getSurveyRepository.getDietPlan();
      response.fold((left) {
        emit(ErrorStateData(errMessage: left.errorMessage!));
      }, (right) {
